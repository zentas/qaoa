...
def generate_and_save_models_threshold(dim, dens, number):
    ...
    save_models_to_file(
        f"./saves/dim_{dim}_dens_{dens}.pkl", models_and_brute_solutions
    )
...
def generate_models_threshold(dim, dens, number):
    ...
    return [
        (matrix, model_generator.brute_force(grm.model_from_matrix(matrix.A)))
        for matrix in matrices
    ]


def read_models_threshold(dim, dens, number):
    matrices_and_brute_solutions = read_models_from_file(
        f"./saves/dim_{dim}_dens_{dens}.pkl"
    )
    ...
    return [
        (grm.model_from_matrix(m.A), b)
        for m, b in matrices_and_brute_solutions[:number]
    ]
